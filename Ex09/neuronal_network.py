...
def backward_pass(param, z, x, output_exact):
    ...
    delta_hidden = np.sum(np.dot(param[2],delta_output[:,None]) * sigmoid_der(z[0]),axis=0)

    return [delta_output.T,delta_hidden]
...
def calc_derivatives_loss_function(inputs, param, delta, x):
    ...
    dWeight_hidden = np.dot(delta[1][:,None], inputs[None,:])
...
if __name__ == "__main__": 
    ...
    exact_output = np.random.rand(1,1)
